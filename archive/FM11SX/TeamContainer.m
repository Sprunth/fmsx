...
- (NSArray *)teamObjects
{
	...
	for (id item in teams) {
		if ([item intValue] < [[[NSApp delegate] valueForKeyPath:@"gameDB.database.teams"] count]) {
			[array addObject:[[[NSApp delegate] valueForKeyPath:@"gameDB.database.teams"] objectAtIndex:[item intValue]]];
		}
	}
